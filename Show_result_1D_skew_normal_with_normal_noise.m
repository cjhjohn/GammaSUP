%**************************************************************************
%--- Plot the results of the case "Skew normal distributions with noise"
...
for ii = 1 : n
    ...
    if( Object_cluster(ii)==1 ), ph = plot( cx, ofv(:,ii) , 'r' ) ; set(ph, 'LineWidth',1); axis( [ -5 , 7 , 0 , 0.85 ] ); end 
    if( Object_cluster(ii)==2 ), ph = plot( cx, ofv(:,ii) , 'g' ) ; set(ph, 'LineWidth',1); axis( [ -5 , 7 , 0 , 0.85 ] ); end
    if( Object_cluster(ii)==3 ), ph = plot( cx, ofv(:,ii) , 'b' ) ; set(ph, 'LineWidth',1); axis( [ -5 , 7 , 0 , 0.85 ] ); end
    if( Object_cluster(ii)> 3 ), ph = plot( cx, ofv(:,ii) , 'k' ) ; set(ph, 'LineWidth',1); axis( [ -5 , 7 , 0 , 0.85 ] ); end
...
%     hold on
    ...
    if( Object_cluster(ii)> 3 ), ph = plot( cx, fv , 'k' ) ; set(ph, 'LineWidth',1); end
    axis( [ -5 , 7 , 0 , 0.85 ] );
        
    hold on

end 
filename = strcat( 'Fig_skew_noise_final.eps' ) ;
saveas(gcf,filename,'psc2') ; 
hold off
